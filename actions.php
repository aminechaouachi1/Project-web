...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (isset($_POST['update'])) {
        $id_comments = $_POST['id_annonces']; // Update this line
        $titre = htmlspecialchars($_POST['actionTitle']);
        $description = htmlspecialchars($_POST['actionDescription']);
    ...
    elseif (isset($_POST['vote'])) {
        // Obtain user identifier (for simplicity, using IP address)
        $userIdentifier = $_SERVER['REMOTE_ADDR'];

        // Sanitize and get vote information
        $announceId = $_POST['announce_id'];
        $voteValue = $_POST['vote'];

        // Insert vote into the 'votes' table
        $stmt = $conn->prepare("INSERT INTO votes (user_identifier, announce_id, vote_value) VALUES (:user_identifier, :announce_id, :vote_value)");
...
if (isset($_GET['edit'])) {
    ...
    $editId = $row['id_annonces']; // Update this line
}
    ...
    <div class="center-container">
        ...
        <div class="rectangle">
            ...
            <div class="edit-delete-buttons">
                <a href="view_announce.php?id=<?= $row['id_annonces'] ?>" class="view-button">View</a>
